...
class NetWork(object):
    ...
    def inference(self):
        ...
        with tf.variable_scope('batch_lstm') as vs_batch_lstm:
            ...
            self.input = tf.placeholder(tf.float32,shape=(None,args.feature_num))
            ...
            with tf.variable_scope('share_lstm') as vs_share_lstm:
                ...
                lstm_out_c = tf.reshape(station_i_state.c,(1,args.lstm_num_units))
                lstm_out_h = tf.reshape(station_i_state.h,(1,args.lstm_num_units))
                # local FC_1 layer
        ...
        return lstm_output,lstm_out_c,lstm_out_h
    ...
    def train(self,station_index):
        ...
        for batch_i in range(args.batch_num):
            all_train_input, all_train_target = dp.next_batch()
            # get the station_index's data
            train_input = np.split(all_train_input,args.station_num,axis=1)[station_index]
            train_target = np.split(all_train_target,args.station_num,axis=1)[station_index]
            ...
            # test
            if batch_i % 5 ==0:
                all_test_input, all_test_target = dp.get_test()
                test_input = np.split(all_test_input,args.station_num,axis=1)[station_index]
                test_target = np.split(all_test_target,args.station_num,axis=1)[station_index]
                feed_dict = {
